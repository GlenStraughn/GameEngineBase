...
uniform vec4 uViewPosition = vec4(0, 0, 10, 0);
...
void main()
{
    ...
    vec4 V = vec4(normalize(uViewPosition.xyz -uViewDirection.xyz), 0);
    // Get reflection angle of view dir. and N
    vec4 R = reflect(-V, N);
	...
	for (int i=0; i<MAX_LIGHTS; i++)
	{
		...
		if (type == NO_LIGHT) {
            ...
            fColor += pow(max(0, dot(L[i].direction, R)), 10);
		}
